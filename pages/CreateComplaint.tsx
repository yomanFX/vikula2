...
export const CreateComplaint: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col max-w-md mx-auto pt-safe-top pb-6">
      ...
      <div className="flex-1 overflow-y-auto pb-32 px-4 space-y-6">
        {step === 1 && (
            <>
                <div className="glass-panel p-6 rounded-3xl text-center backdrop-blur-3xl shadow-[0_0_40px_rgba(0,0,0,0.3)]">
                    <p className="text-white/60 text-xs font-bold uppercase tracking-wider mb-2">{isGoodDeedMode ? 'Автор' : 'Виновник'}</p>
                    <h3 className="text-3xl font-black text-primary drop-shadow-[0_0_10px_rgba(96,165,250,0.6)]">{accusedUser}</h3>
                </div>
                        ...
                        {CATEGORIES.map(cat => (
                            <div key={cat.id} onClick={() => { setCategory(cat); setCustomEmoji(''); }}
                                className={`glass-panel aspect-square rounded-2xl flex flex-col items-center justify-center cursor-pointer transition-all duration-300 border-2
                                    ${category?.id === cat.id && !customEmoji ? 'border-primary bg-primary/20 scale-105 shadow-[0_0_20px_rgba(96,165,250,0.3)]' : 'border-transparent hover:bg-white/5'}`}
                            >
                                <span className="text-4xl mb-2 filter drop-shadow-md">{cat.icon}</span>
                         ...
                         <div className={`glass-panel aspect-square rounded-2xl flex flex-col items-center justify-center cursor-pointer border-2 ${customEmoji ? 'border-primary bg-primary/20' : 'border-transparent'}`}>
                            <input type="text" value={customEmoji} onChange={(e) => { setCustomEmoji(e.target.value); setCategory(null); }} className="bg-transparent text-center text-4xl w-full outline-none text-white" placeholder="➕" maxLength={2} />
                        </div>
                    </div>
                )}
                ...
                <div className="space-y-4">
                    <textarea 
                        ...
                        className="glass-input w-full p-5 min-h-[140px] text-base focus:ring-1 focus:ring-primary/50 outline-none placeholder-white/30"
                        placeholder={isGoodDeedMode ? "Опишите ваш героический поступок..." : "Что этот человек натворил?"}
                    />

                    <div onClick={() => fileInputRef.current?.click()} className="glass-panel border-2 border-dashed border-white/20 rounded-2xl h-32 flex flex-col items-center justify-center cursor-pointer hover:bg-white/5 transition-colors relative overflow-hidden">
                        {image ? (
                             ...
                             <>
                                ...
                                <span className="text-xs font-bold text-white/60 uppercase">Добавить фото</span>
                             </>
                        )}
        ...
        {step === 2 && !isGoodDeedMode && (
            <>
                <div className="glass-panel p-6 rounded-3xl border border-red-500/30 bg-red-500/5">
                    <div className="flex justify-between items-center mb-4">
                         <span className="text-xs font-bold text-white/60 uppercase">Уровень боли</span>
                         <span className="text-3xl font-black text-red-400 drop-shadow-[0_0_10px_rgba(248,113,113,0.5)]">-{penaltyPoints}</span>
                    </div>
                    <input type="range" min="5" max="100" step="5" value={penaltyPoints} onChange={(e) => setPenaltyPoints(Number(e.target.value))} className="w-full h-2 bg-white/20 rounded-lg appearance-none cursor-pointer accent-red-500" />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    {COMPENSATIONS.map(comp => (
                        <div key={comp.id} onClick={() => { setCompensation(comp); setCustomCompensation(''); }}
                            className={`glass-panel p-4 rounded-2xl flex flex-col items-center justify-center gap-3 cursor-pointer transition-all border-2
                                ${compensation?.id === comp.id && !customCompensation ? 'border-primary bg-primary/10 shadow-[0_0_15px_rgba(96,165,250,0.3)]' : 'border-transparent hover:bg-white/5'}`}
                        >
                    ...
                    <div onClick={() => { setCompensation(null); customCompInputRef.current?.focus(); }} className={`col-span-2 glass-panel p-4 rounded-2xl flex items-center gap-4 cursor-pointer border-2 ${customCompensation ? 'border-primary' : 'border-transparent'}`}>
                         <div className="size-10 rounded-full bg-white/10 flex items-center justify-center"><span className="material-symbols-outlined text-white">edit</span></div>
                         <input ref={customCompInputRef} type="text" className="bg-transparent w-full outline-none font-bold placeholder-white/30 text-white" placeholder="Свой вариант..." value={customCompensation} onChange={(e) => { setCustomCompensation(e.target.value); setCompensation(null); }} />
                    </div>
                </div>
            </>
        )}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 z-30 pb-8 flex justify-center pointer-events-none">
        <button onClick={handleNext} disabled={isSubmitting} className="pointer-events-auto w-full max-w-[480px] h-14 bg-primary text-white text-lg font-bold rounded-2xl shadow-[0_0_30px_rgba(96,165,250,0.4)] flex items-center justify-center gap-2 active:scale-95 transition-transform border border-white/20">
